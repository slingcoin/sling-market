...
//step 2.) Top FASTSEND_SIGNATURES_TOTAL masternodes, open connect to top 1 masternode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 masternode, waits for FASTSEND_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'

void ProcessMessageFastSend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (fLiteMode) return; //disable all coinmix/masternode related functionality
    if (!IsSporkActive(SPORK_2_FASTSEND)) return;
    if (!masternodeSync.IsBlockchainSynced()) return;

    if (strCommand == "ix") {
        //LogPrintf("ProcessMessageFastSend::ix\n");
        ...
        BOOST_FOREACH (const CTxOut o, tx.vout) {
            ...
            if (!o.scriptPubKey.IsNormalPaymentScript() && !o.scriptPubKey.IsUnspendable()) {
                LogPrintf("ProcessMessageFastSend::ix - Invalid Script %s\n", tx.ToString().c_str());
                return;
            }
        }
        ...
        if (fAccepted) {
            ...
            LogPrintf("ProcessMessageFastSend::ix - Transaction Lock Request: %s %s : accepted %s\n",
            ...
            LogPrintf("ProcessMessageFastSend::ix - Transaction Lock Request: %s %s : rejected %s\n",
                pfrom->addr.ToString().c_str(), pfrom->cleanSubVer.c_str(),
                tx.GetHash().ToString().c_str());
            ...
            if (i != mapTxLocks.end()) {
                ...
                if ((*i).second.CountSignatures() >= FASTSEND_SIGNATURES_REQUIRED) {
                    if (!CheckForConflictingLocks(tx)) {
                        LogPrintf("ProcessMessageFastSend::ix - Found Existing Complete IX Lock\n");
    ...
    } else if (strCommand == "txlvote") //FastSend Lock Consensus Votes
    {
        CConsensusVote ctx;
        vRecv >> ctx;
            ...
            */
            if (!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)) {
                ...
                if (mapUnknownVotes[ctx.vinMasternode.prevout.hash] > GetTime() &&
                    ...
                    LogPrintf("ProcessMessageFastSend::ix - masternode is spamming transaction votes: %s %s\n",
...
bool IsIXTXValid(const CTransaction& txCollateral)
{
    ...
    if (nValueOut > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
        LogPrint("fastsend", "IsIXTXValid - Transaction value too high - %s\n", txCollateral.ToString().c_str());
        return false;
    }

    if (missingTx) {
        LogPrint("fastsend", "IsIXTXValid - Unknown inputs in IX transaction - %s\n", txCollateral.ToString().c_str());
        /*
    ...
    if (nValueIn - nValueOut < COIN * 0.01) {
        LogPrint("fastsend", "IsIXTXValid - did not include enough fees in transaction %d\n%s\n", nValueOut - nValueIn, txCollateral.ToString().c_str());
        return false;
    }
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    */
    ...
    if (!mapTxLocks.count(tx.GetHash())) {
        ...
        LogPrint("fastsend", "CreateNewLock - Transaction Lock Exists %s !\n", tx.GetHash().ToString().c_str());
    }
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetMasternodeRank(activeMasternode.vin, nBlockHeight, MIN_FASTSEND_PROTO_VERSION);

    if (n == -1) {
        LogPrint("fastsend", "FastSend::DoConsensusVote - Unknown Masternode\n");
        return;
    }

    if (n > FASTSEND_SIGNATURES_TOTAL) {
        LogPrint("fastsend", "FastSend::DoConsensusVote - Masternode not in the top %d (%d)\n", FASTSEND_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    LogPrint("fastsend", "FastSend::DoConsensusVote - In the top %d (%d)\n", FASTSEND_SIGNATURES_TOTAL, n);
    ...
    if (!ctx.Sign()) {
        LogPrintf("FastSend::DoConsensusVote - Failed to sign consensus vote\n");
        return;
    }
    if (!ctx.SignatureValid()) {
        LogPrintf("FastSend::DoConsensusVote - Signature invalid\n");
        return;
    }
...
{
    int n = mnodeman.GetMasternodeRank(ctx.vinMasternode, ctx.nBlockHeight, MIN_FASTSEND_PROTO_VERSION);

    CMasternode* pmn = mnodeman.Find(ctx.vinMasternode);
    if (pmn != NULL)
        LogPrint("fastsend", "FastSend::ProcessConsensusVote - Masternode ADDR %s %d\n", pmn->addr.ToString().c_str(), n);

    if (n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("fastsend", "FastSend::ProcessConsensusVote - Unknown Masternode\n");
        mnodeman.AskForMN(pnode, ctx.vinMasternode);
        return false;
    }

    if (n > FASTSEND_SIGNATURES_TOTAL) {
        LogPrint("fastsend", "FastSend::ProcessConsensusVote - Masternode not in the top %d (%d) - %s\n", FASTSEND_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if (!ctx.SignatureValid()) {
        LogPrintf("FastSend::ProcessConsensusVote - Signature invalid\n");
    ...
    if (!mapTxLocks.count(ctx.txHash)) {
        LogPrintf("FastSend::ProcessConsensusVote - New Transaction Lock %s !\n", ctx.txHash.ToString().c_str());
        ...
        LogPrint("fastsend", "FastSend::ProcessConsensusVote - Transaction Lock Exists %s !\n", ctx.txHash.ToString().c_str());
    ...
    if (i != mapTxLocks.end()) {
        ...
        LogPrint("fastsend", "FastSend::ProcessConsensusVote - Transaction Lock Votes %d - %s !\n", (*i).second.CountSignatures(), ctx.GetHash().ToString().c_str());

        if ((*i).second.CountSignatures() >= FASTSEND_SIGNATURES_REQUIRED) {
            LogPrint("fastsend", "FastSend::ProcessConsensusVote - Transaction Lock Is Complete %s !\n", (*i).second.GetHash().ToString().c_str());
    ...
    BOOST_FOREACH (const CTxIn& in, tx.vin) {
        if (mapLockedInputs.count(in.prevout)) {
            if (mapLockedInputs[in.prevout] != tx.GetHash()) {
                LogPrintf("FastSend::CheckForConflictingLocks - found two complete conflicting locks - removing both. %s %s", tx.GetHash().ToString().c_str(), mapLockedInputs[in.prevout].ToString().c_str());
...
bool CConsensusVote::SignatureValid()
{
    ...
    if (pmn == NULL) {
        LogPrintf("FastSend::CConsensusVote::SignatureValid() - Unknown Masternode\n");
        return false;
    }

    if (!CoinMixSigner.VerifyMessage(pmn->pubKeyMasternode, vchMasterNodeSignature, strMessage, errorMessage)) {
        LogPrintf("FastSend::CConsensusVote::SignatureValid() - Verify message failed\n");
        return false;
    }
...
bool CConsensusVote::Sign()
{
    ...
    if (!CoinMixSigner.SignMessage(strMessage, errorMessage, vchMasterNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if (!CoinMixSigner.VerifyMessage(pubkey2, vchMasterNodeSignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetMasternodeRank(vote.vinMasternode, vote.nBlockHeight, MIN_FASTSEND_PROTO_VERSION);
