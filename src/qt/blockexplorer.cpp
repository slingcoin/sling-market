...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::SLING, nValue);
...
{
    std::string genesisblockhash = "00000328f9124ad164c2a7e742c97a9dfa1e91be174d7e54c8181fc123c5da75";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (sling.conf).");
            QMessageBox::warning(this, "Sling Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
