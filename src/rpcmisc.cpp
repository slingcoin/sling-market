...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"coinmix_balance\": xxxxxx, (numeric) the anonymized sling balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in sling/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in sling/kb\n"
    ...
    if (pwalletMain) {
        ...
        if (!fLiteMode)
            obj.push_back(Pair("coinmix_balance", ValueFromAmount(pwalletMain->GetAnonymizedBalance())));
    }
#endif
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given sling address.\n"
            "\nArguments:\n"
            "1. \"slingaddress\"     (string, required) The sling address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"slingaddress\", (string) The sling address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Sling address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are sling addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) sling address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"slingaddress\"  (string, required) The sling address to use for the signature.\n"
