...
bool IsSporkActive(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_2_FASTSEND) r = SPORK_2_FASTSEND_DEFAULT;
        if (nSporkID == SPORK_3_FASTSEND_BLOCK_FILTERING) r = SPORK_3_FASTSEND_BLOCK_FILTERING_DEFAULT;
...
bool CSporkManager::CheckSignature(CSporkMessage& spork)
{
    ...
    if (!CoinMixSigner.VerifyMessage(pubkey, spork.vchSig, strMessage, errorMessage)) {
        return false;
    }
...
bool CSporkManager::Sign(CSporkMessage& spork)
{
    ...
    if (!CoinMixSigner.SetKey(strMasterPrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CMasternodePayments::Sign - ERROR: Invalid masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }

    if (!CoinMixSigner.SignMessage(strMessage, errorMessage, spork.vchSig, key2)) {
        LogPrintf("CMasternodePayments::Sign - Sign message failed");
        return false;
    }

    if (!CoinMixSigner.VerifyMessage(pubkey2, spork.vchSig, strMessage, errorMessage)) {
...
{
    if (strName == "SPORK_2_FASTSEND") return SPORK_2_FASTSEND;
...
{
    if (id == SPORK_2_FASTSEND) return "SPORK_2_FASTSEND";
    if (id == SPORK_3_FASTSEND_BLOCK_FILTERING) return "SPORK_3_FASTSEND_BLOCK_FILTERING";
