...
Value coinmix(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "coinmix <slingaddress> <amount>\n"
    ...
    if (params[0].get_str() == "auto") {
        if (fMasterNode)
            return "CoinMix is not supported from masternodes";

        return "DoAutomaticDenominating " + (CoinMixPool.DoAutomaticDenominating() ? "successful" : ("failed: " + CoinMixPool.GetStatus()));
    }

    if (params[0].get_str() == "reset") {
        CoinMixPool.Reset();
        return "successfully reset coinmix";
    }
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("state", CoinMixPool.GetState()));
    obj.push_back(Pair("entries", CoinMixPool.GetEntriesCount()));
    obj.push_back(Pair("entries_accepted", CoinMixPool.GetCountEntriesAccepted()));
    return obj;
}
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in sling.conf\n"
...
Value masternodelist(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1)) {
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode Sling address\n"
