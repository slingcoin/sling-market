...
class CTxIn;
class CCoinMixPool;
class CCoinMixSigner;
class CMasterNodeVote;
class CBitcoinAddress;
class CCoinMixQueue;
class CCoinMixBroadcastTx;
...
#define COINMIX_QUEUE_TIMEOUT 30
#define COINMIX_SIGNING_TIMEOUT 15

// used for anonymous relaying of inputs/outputs/sigs
#define COINMIX_RELAY_IN 1
#define COINMIX_RELAY_OUT 2
#define COINMIX_RELAY_SIG 3

static const int64_t COINMIX_COLLATERAL = (10 * COIN);
static const int64_t COINMIX_POOL_MAX = (99999.99 * COIN);

extern CCoinMixPool CoinMixPool;
extern CCoinMixSigner CoinMixSigner;
extern std::vector<CCoinMixQueue> vecCoinMixQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CCoinMixBroadcastTx> mapCoinMixBroadcastTxes;
extern CActiveMasternode activeMasternode;

/** Holds an CoinMix input
 */
...
// A clients transaction in the coinmix pool
class CCoinMixEntry
{
public:
    bool isSet;
    ...
    /// Add entries to use for CoinMix
    ...
    bool IsExpired()
    {
        return (GetTime() - addedTime) > COINMIX_QUEUE_TIMEOUT; // 120 seconds
    }
};


/**
 * A currently inprogress CoinMix merge and denomination information
 */
class CCoinMixQueue
{
public:
    ...
    /// Is this CoinMix expired?
    bool IsExpired()
    {
        return (GetTime() - time) > COINMIX_QUEUE_TIMEOUT; // 120 seconds
    }
...
/** Helper class to store CoinMix transaction (tx) information.
 */
class CCoinMixBroadcastTx
{
public:
...
{
public:
    /// Is the inputs associated with this public key? (and there is 7331 SLING - checking if valid masternode)
...
/** Used to keep track of current status of CoinMix pool
 */
class CCoinMixPool
{
private:
    mutable CCriticalSection cs_coinmix;

    std::vector<CCoinMixEntry> entries; // Masternode/clients entries
...
public:
    ...
    CCoinMixPool()
    {
        /* CoinMix uses collateral addresses to trust parties entering the pool
    ...
    /** Process a CoinMix message using the CoinMix protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | CoinMix Acceptable
     *        dsc      | CoinMix Complete
     *        dsf      | CoinMix Final tx
     *        dsi      | CoinMix vIn
     *        dsq      | CoinMix Queue
     *        dss      | CoinMix Signal Final Tx
     *        dssu     | CoinMix status update
     *        dssub    | CoinMix Subscribe To
     * \param vRecv
     */
    void ProcessMessageCoinMix(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);

    void InitCollateralAddress()
    {
        SetCollateralAddress(Params().CoinMixPoolDummyAddress());
    }
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)) {
            LogPrint("coinmix", "CCoinMixPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Masternode. \n");
            return;
        }

        LogPrintf("CCoinMixPool::UpdateState() == %d | %d \n", state, newState);
        if (state != newState) {
    ...
    /// Passively run CoinMix in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun = false);
    bool PrepareCoinMixDenominate();

    /// Check for process in CoinMix
    void Check();
    ...
    void SendCoinMixDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    /// Get Masternode updates about the progress of CoinMix
