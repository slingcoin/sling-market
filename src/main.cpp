...
#if defined(NDEBUG)
#error "Sling cannot be compiled without assertions."
#endif
...
int GetInputAgeIX(uint256 nTXHash, CTxIn& vin)
{
    ...
    if (nResult < 6) {
        ...
        if (sigs >= FASTSEND_SIGNATURES_REQUIRED) {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (mapCoinMixBroadcastTxes.count(hash)) {
...
CAmount GetBlockValue(int nHeight, CAmount nFees, bool fProofOfStake)
{
    ...
    if (!fProofOfStake) {
        // This is a PoW block
        if (nHeight == 1) {
            nSubsidy = STATIC_SWAP_BLOCK_AMOUNT; //First block to payout swap for orginal chain Sling coin holders.
        }
        else if (nHeight > 1 && nHeight < Params().FirstRewardBlock()) {
            nSubsidy = 0;
        }
        else if (nHeight >= Params().FirstRewardBlock()) {
            nSubsidy = STATIC_POW_PAYMENT_AMOUNT + STATIC_MASTERNODE_PAYMENT_AMOUNT + nFees;
        }
    }
    else {
        // This is a PoS block
        if (nHeight > 0 && nHeight < Params().FirstRewardBlock()) {
            nSubsidy = 0;
        }
        else if (nHeight >= Params().FirstRewardBlock()) {
            nSubsidy = STATIC_POS_PAYMENT_AMOUNT + nFees;
        }
    }
...
//TODO: CryptoDJ, adjust masternode payments
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                    std::max(chainActive.Tip()->GetBlockTime(), pindexBestHeader->GetBlockTime()) < GetTime() - DEFAULT_MAX_TIP_AGE);
...
/*
    ...
    Remove conflicting blocks for successful FastSend transaction locks
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (IsSporkActive(SPORK_3_FASTSEND_BLOCK_FILTERING)) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            if (!tx.IsCoinBase()) {
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_DSTX:
        return mapCoinMixBroadcastTxes.count(inv.hash);
    case MSG_BLOCK:
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_DSTX) {
                    if (mapCoinMixBroadcastTxes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mapCoinMixBroadcastTxes[inv.hash].tx << mapCoinMixBroadcastTxes[inv.hash].vin << mapCoinMixBroadcastTxes[inv.hash].vchSig << mapCoinMixBroadcastTxes[inv.hash].sigTime;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                ...
                if (!mapCoinMixBroadcastTxes.count(tx.GetHash())) {
                    ...
                    mapCoinMixBroadcastTxes.insert(make_pair(tx.GetHash(), dstx));
                }
            }
        }
    ...
    // getaddr message mitigates the attack.
        ...
        CoinMixPool.ProcessMessageCoinMix(pfrom, strCommand, vRecv);
        mnodeman.ProcessMessage(pfrom, strCommand, vRecv);
        budget.ProcessMessage(pfrom, strCommand, vRecv);
        masternodePayments.ProcessMessageMasternodePayments(pfrom, strCommand, vRecv);
        ProcessMessageFastSend(pfrom, strCommand, vRecv);
...
{
    {
        ...
        //TODO: CryptoDJ, change use of Params().PoSTargetSpacing()
        if (!pto->fDisconnect && state.vBlocksInFlight.size() > 0 && state.vBlocksInFlight.front().nTime < nNow - 500000 * Params().PoSTargetSpacing() * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
